...
export class ValidatorHandler {
    ...
    public static async Create(address: string, network: string): Promise<ValidatorHandler> {
        const client = await makeClient(directory.rpcUrl(network));
        const chain = await directory.getChain(network);
        const handler = new ValidatorHandler(address, network, client, chain.chain.denom, chain.chain.decimals);
        return handler;
    }

    getDelegators = async (): Promise<any> => {
        const delegators = await this._client.staking.validatorDelegations(this.address);
        return delegators;
    }
}